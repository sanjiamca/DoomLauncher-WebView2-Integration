...
namespace DoomLauncher
{
    public partial class SettingsForm : Form
    {
        ...
        // Senin istediğin özel link:
        private const string NEW_SEARCH_URL = "https://www.doomworld.com/idgames/index.php?search=1&field=filename&word={0}&sort=time&order=asc&page=1";
        ...
        private void PopulateConfiguration()
        {
            ...
            {
                // 1. Mirror Url'yi gizliyoruz
                if (config.Name == "MirrorUrl") 
                    continue; 

                // 2. İsmi "Search URL" yapıyoruz
                string labelText = AddSpaceBetweenWords(config.Name);
                if (config.Name.Equals("ApiPage", StringComparison.OrdinalIgnoreCase))
                {
            ...
            tabControl.TabPages[0].AutoScroll = true;
            tabControl.TabPages[0].Controls.Add(tblMain);

            int contentHeight = height + dpiScale.ScaleIntY(180); 
            int screenLimit = Screen.PrimaryScreen.WorkingArea.Height - 100;
            
            this.Height = Math.Min(contentHeight, screenLimit);
            this.Width = dpiScale.ScaleIntX(500); 
        }
        ...
        private void HandleTextBox(TableLayoutPanel tblMain, IConfigurationData config)
        {
            ...
            // Eğer ayar "ApiPage" ise ve değeri eski "api/api.php" ise (veya boşsa),
            // OTOMATİK OLARAK senin yeni linkini yazıyoruz.
            if (config.Name == "ApiPage")
            {
                if (txt.Text == "api/api.php" || string.IsNullOrWhiteSpace(txt.Text))
                {
                    txt.Text = NEW_SEARCH_URL;
                }
            }
            // ---------------------------------------------------------
